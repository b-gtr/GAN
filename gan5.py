...
def image_dataset(IMAGE_SIZE, DATA_PATH):
    ...
    subset_indices = range(200)  # or use a random subset via torch.randperm
...
def meta_train_epoch_discriminator(meta_epoch, meta_model, meta_train_dataloader, meta_loss_func, meta_optimizer, device, real_label, fake_label, LATENT_DIM, netG):
    ...
    for meta_train_idx, data in enumerate(meta_train_dataloader, 0):
        ...
        if meta_train_idx % 50 == 0:
            print(f'Discriminator Meta Epoch {meta_epoch}, Batch {meta_train_idx}: Loss {errD.item()}')
    return meta_model, meta_epoch_running_loss / len(meta_train_dataloader)

def meta_train_epoch_generator(meta_epoch, meta_model, meta_train_dataloader, meta_loss_func, meta_optimizer, DEVICE, real_label, LATENT_DIM, netG, netD):
    meta_epoch_running_loss = 0
    for meta_train_idx, data in enumerate(meta_train_dataloader, 0):
        ...
        errG = meta_loss_func(output, real_label)
        
        #meta_optimizer.zero_grad()  # Add this line
        errG.backward()
        meta_optimizer.step()  # Add this line
    
        meta_epoch_running_loss += errG.item()
        if meta_train_idx % 50 == 0:
            print(f'Generator the meta training batch loss at meta epoch of  {meta_epoch} idx of {meta_train_idx} is {errG}')
    return meta_model, meta_epoch_running_loss / len(meta_train_dataloader)

def train_epoch_discriminator(meta_epoch, epoch, model, train_dataloader, loss_func, optimizer, device, real_label, fake_label, LATENT_DIM, netG):
    ...
    for train_idx, data in enumerate(train_dataloader, 0):
        ...
        if train_idx % 50 == 0:
            print(f'Discriminator Epoch {epoch}, Batch {train_idx}: Loss {errD.item()}')
    return model, epoch_running_loss / len(train_dataloader)

def train_epoch_generator(meta_epoch, epoch, model, train_dataloader, loss_func, optimizer, DEVICE, real_label, LATENT_DIM, netD):
    epoch_running_loss = 0
    for train_idx, data in enumerate(train_dataloader, 0):
        features = data[0].to(DEVICE)
        b_size = features.size(0)
        ...
        if train_idx % 50 == 0:
            print(f'Generator Epoch {epoch}, Batch {train_idx}: Loss {errG.item()}')
    return model, epoch_running_loss / len(train_dataloader)


def main():
    DATA_PATH = r"C:\Users\q667874\BMW Group\MS-20_Code\ninja_cell\face_test"
    IMAGE_SIZE = 512
    NC = 1
    NZ = 100
    NGF = 64
    ...
    schedulerG = torch.optim.lr_scheduler.StepLR(optimizerG, step_size=100, gamma=0.1)
    ...
    for epoch in range(EPOCHS):
        ...
        for meta_epoch in range(meta_epochs): 
            meta_netD, meta_loss = meta_train_epoch_discriminator(
        ...
        for t_epoch in range(train_epochs):
            netD, train_loss = train_epoch_discriminator(
                meta_epoch=meta_epoch,
                epoch=t_epoch,
                model=netD,
        ...
        for meta_epoch in range(meta_epochs): 
            meta_netG, meta_G_loss = meta_train_epoch_generator(
                meta_epoch=meta_epoch,
        ...
        for t_epoch in range(train_epochs):
            netG, train_G_loss = train_epoch_generator(
                meta_epoch=meta_epoch,
                epoch=t_epoch,
                model=netG,
                train_dataloader=dataloader,
        ...
        if (epoch % 160 == 0):
            ...
            # Create image grid and save to file
